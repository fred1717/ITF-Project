# Data source to dynamically fetch the subnet ID by tag and AZ data "aws_subnet" "app_subnet" {
    ...
    vpc_id =
    var.vpc_id service_name
    = "com.amazonaws.us-east-1.ssmmessages"
    vpc_endpoint_type =
    "Interface" subnet_ids
    = [
    ...
    load_balancer_arn =
    aws_lb.app_alb.arn port
    = 80
    protocol =
    "HTTP" default_action
    {
        type =
        "forward" target_group_arn
        = aws_lb_target_group.app_tg.arn
    }
} # Target Group for AppTier EC2 instance resource "aws_lb_target_group" "app_tg" {
    name =
    "TargetGroup-RegionA" port
    = 80
    protocol =
    "HTTP" target_type
    = "instance"
    vpc_id =
    var.vpc_id health_check
    {
        path =
        "/health" protocol
        = "HTTP"
    ...
    target_group_arn =
    aws_lb_target_group.app_tg.arn target_id
    = aws_instance.AppTier_Private-ITF_AZ1.id
    port =
    80
}
